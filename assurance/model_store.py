...
def _validate_validation(
    ...
    *,
    ...
    if report.get("policy_sha256") != policy_sha:
        raise ValueError("validation report was produced under a different calibration policy")
...
def verify_bundle(bundle_dir: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        or manifest.get("bundle_version") != BUNDLE_FORMAT_VERSION
    ):
    ...
    policy_sha = sha256_file(resolved["policy"])
    validation_sha = sha256_file(resolved["validation"])
    ...
    schema, contract_hash, _ = validate_model_input_schema_binding(
        resolved["model"], resolved["input_schema"]
    )
    ...
    _validate_validation(
        _read_json(resolved["validation"]),
        model_sha=model_sha,
    ...
    if manifest.get("policy_sha256") != policy_sha or manifest.get("validation_sha256") != validation_sha:
...
def build_bundle(
    ...
    *,
    ...
    output = assert_safe_tree_target(
        output,
        protected_paths=[model, policy, validation, schema_path],
        operation="deployment bundle replacement",
    )
...
def _read_state(path: Path) -> dict[str, Any]:
    ...
    if not _is_sha256(state.get("active_bundle_id")):
        raise ValueError("deployment state has no valid active bundle")
    return state
...
def promote_bundle(
    candidate_bundle: str | Path, store: str | Path, state_path: str | Path
) -> dict[str, Any]:
    ...
    if state_path.exists():
        ...
        active_id = str(old["active_bundle_id"])
        active_manifest = verify_bundle(store / active_id)
        if active_manifest.get("bundle_id") != active_id:
            raise ValueError("current active deployment state does not match stored bundle identity")
        if active_id == bundle_id:
            return old
        previous, generation = active_id, int(old["generation"]) + 1
    else:
...
def resolve_bundle(
    store: str | Path, state_path: str | Path, *, slot: str = "active"
) -> dict[str, Any]:
    ...
    if not _is_sha256(bundle_id):
        raise FileNotFoundError(f"no valid {slot} deployment bundle is available")
    bundle_id = str(bundle_id)
    bundle_dir = store / bundle_id
    manifest = verify_bundle(bundle_dir)
    if manifest.get("bundle_id") != bundle_id:
        raise ValueError(f"deployment state {slot} bundle id does not match bundle manifest")
...
def main() -> None:
    ...
    roll.add_argument("--state", type=Path, required=True)
    resolve = sub.add_parser("resolve")
    resolve.add_argument("--store", type=Path, required=True)
    resolve.add_argument("--state", type=Path, required=True)
    resolve.add_argument("--slot", choices=["active", "previous"], default="active")
